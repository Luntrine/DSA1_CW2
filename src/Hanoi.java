...
public class Hanoi {
	...
	/**
	 * This method moves a single ring from one tower to another given the
	 * parameters of which tower to move the ring from to which tower it's supposed
	 * to move the ring to.
	 * 
	 * This method has a big O complexity of 1.
	 * 
	 * @param from the tower to move the ring from.
	 * @param to the tower to move the ring to.
	 */
	public void moveOneRing(int from, int to) {

		// Checks whether each tower is empty before moving anything to or from them.
		checkTower(to);
		checkTower(from);

		// Pushes a ring to the "to" tower while popping one from the "from" tower.
	...
	/*
	 * This method moves a pile of rings of size n from one tower to another given
	 * the parameters of which tower to move the ring from to which tower it's
	 * supposed to move the ring to as well as the tower it's supposed to use as the
	 * intermediary. It's a recursive method.
	 * 
	...
	public void movePile(int n, int from, int to, int via) {
		...
		/*
		 * If there's only one ring left to move, it moves the ring from the tower it's
		 * currently on to the tower it needs to go to and returns to the previous
		 * movePile() on the call stack. If there are no more movePile() methods on the
		 * call stack, return concludes the recursive method.
		 */
		...
		/*
		 * If there's more than one ring to move, the method recursively goes through an
		 * algorithm by calling itself as well as an intermittent call of moveOneRing()
